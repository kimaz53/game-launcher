      ...
      <div className={cn('mb-3 flex gap-2', justifyClass)}>
        <div
          className={cn(
            panel,
            columnListClass,
            'max-h-[min(52vh,28rem)] overflow-y-auto overflow-x-hidden [scrollbar-width:thin] bg-theme-secondary/50 backdrop-blur-xl',
          )}
...
export default function Home() {
  ...
  const libraryShellClass =
    ''
  const quickDockClass = cn(
    'rounded-2xl border p-2 backdrop-blur-xl',
    isLightMode
      ? 'border-black/8 bg-gradient-to-b from-theme-card/60 to-theme-sidebar/50 shadow-[0_12px_40px_-12px_rgba(0,0,0,0.15)] ring-1 ring-black/5'
      : 'border-white/10 bg-gradient-to-b from-theme-card/50 to-theme-sidebar/40 shadow-[0_12px_40px_-12px_rgba(0,0,0,0.45)] ring-1 ring-white/5',
  )
    ...
    >
      ...
      {backgroundImageSrc ? (
        <div className={cn(
          ...
          isLightMode
            ? 'from-theme-app/35 via-theme-app/25 to-theme-app/50 backdrop-blur-[6px]'
            : 'from-theme-app/85 via-theme-app/70 to-theme-app/95 backdrop-blur-[2px]',
        )} />
      ) : (
      ...
      <div className={cn(
        ...
        isLightMode
          ? 'bg-[radial-gradient(ellipse_at_center,transparent_0%,rgba(0,0,0,0.12)_100%)]'
          : 'bg-[radial-gradient(ellipse_at_center,transparent_0%,rgba(0,0,0,0.35)_100%)]',
      )} />

      <header className={cn(
        'relative z-10 mx-3 mt-2 flex min-h-[3.25rem] shrink-0 items-center gap-3 rounded-2xl border px-3 py-2 backdrop-blur-xl md:mx-4 md:mt-3 md:min-h-[3.5rem] md:gap-4 md:px-4',
        isLightMode
          ? 'border-black/8 bg-theme-sidebar/55 shadow-[0_8px_32px_-8px_rgba(0,0,0,0.12)]'
          : 'border-theme-border/10 bg-theme-sidebar/45 shadow-[0_8px_32px_-8px_rgba(0,0,0,0.4)]',
      )}>
        <div className="flex min-w-0 flex-1 items-center justify-start md:flex-none">
          {logoImageSrc ? (
            ...
            <div className="truncate bg-gradient-to-r from-theme-text to-theme-muted bg-clip-text text-2xl font-bold tracking-wide text-transparent md:text-2xl">
              {shopName}
            </div>
          )}
        </div>
        <div className="relative flex min-w-0 max-w-md flex-1 items-center gap-2 ml-auto">
          {/* <Search className="pointer-events-none absolute left-3 top-1/2 z-10 h-4 w-4 -translate-y-1/2 text-theme-muted" aria-hidden /> */}
          <Input
            className={cn(
              'h-10 min-w-0 flex-1 rounded-xl border pl-3 pr-3 text-sm shadow-inner transition-[box-shadow,background] placeholder:text-theme-muted focus-visible:border-theme-primary/40 focus-visible:ring-2 focus-visible:ring-theme-primary/25',
              isLightMode
                ? 'border-black/10 bg-theme-card/70'
                : 'border-white/10 bg-theme-card/80',
            )}
            value={searchInput}
      ...
      >
            ...
            <aside className="flex items-center gap-2">
              ...
              <div className={cn('flex max-w-full overflow-x-auto p-1 items-center gap-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden')}>
                {quickAccessGames.map((g) => (
                  ...
                  >
                    <GameArtwork quickAccess className='!bg-transparent border-none' game={g} iconSize="small" tagsPosition={tagsPosition} showTags={false} />
                  </Button>
                ))}
              </div>
            </aside>
        ...
        {showQuickAccess && quickIsLeft && quickAccessGames.length > 0 ? (
          <div className={cn('order-[-1] flex min-h-0 flex-col', quickSlotAlignClass, quickDockClass)}>
            <aside className="flex min-h-0 flex-col items-center gap-2">
              {showQuickAccessTitle ? (
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-theme-muted">Quick access</span>
              ) : null}
              <div
          ...
          <main className="flex min-h-0 min-w-0 flex-1 flex-col gap-2">
            ...
            <div className="flex min-h-0 min-w-0 flex-1 gap-3">
              ...
              <div
                ...
                className={cn(
                  'relative min-h-0 min-w-0 p-1 flex-1 overflow-y-auto [scrollbar-width:thin] overscroll-contain',
                  libraryShellClass,
                )}
              >
                {(() => {
                  ...
                  const minTileWidthPx =
                    iconSize === 'small' ? 90 : iconSize === 'large' ? 149 : 129
                  const rowHeightPx =
                    iconSize === 'small' ? 85 : iconSize === 'large' ? 265 : 225
                          ...
                          >
                            <Button
                              ...
                              className={cn(
                                ...
                                isLightMode
                                  ? 'border border-black/6 bg-theme-card/35 text-inherit shadow-sm ring-1 ring-black/5 transition-all duration-200'
                                  : 'border border-white/5 bg-theme-card/20 text-inherit shadow-sm ring-1 ring-black/20 transition-all duration-200',
                                isLightMode
                                  ? '!bg-theme-secondary/5 hover:-translate-y-0.5 hover:!border-theme-primary/30 hover:!bg-theme-secondary/55 hover:shadow-lg hover:shadow-black/10'
                                  : '!bg-theme-primary/5 hover:-translate-y-0.5 hover:!border-theme-primary/25 hover:!bg-theme-primary/30 hover:shadow-lg hover:shadow-black/30',
        ...
        {showQuickAccess && quickIsStackedBottom && quickAccessGames.length > 0 ? (
          <div className={cn('flex px-5', quickSlotAlignClass)}>
            <aside className="flex items-center gap-2">
              {showQuickAccessTitle ? (
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-theme-muted">Quick access</span>
              ) : null}
              <div className={cn(quickDockClass, 'flex max-w-full overflow-x-auto p-1 items-center gap-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden')}
                style={
                  libraryGridWidthPx != null && libraryGridWidthPx > 0
                    ? { maxWidth: libraryGridWidthPx }
                    : undefined
                }
              >
                {quickAccessGames.map((g) => (
      ...
      {showFooter ? (
        <footer className={cn(
          'relative z-10 mx-3 mb-2 flex items-center gap-3 rounded-2xl border px-3 py-2.5 text-sm backdrop-blur-xl md:mx-4',
          isLightMode
            ? 'border-black/8 bg-theme-sidebar/55 shadow-[0_-8px_32px_-12px_rgba(0,0,0,0.1)]'
            : 'border-white/10 bg-theme-sidebar/40 shadow-[0_-8px_32px_-12px_rgba(0,0,0,0.35)]',
        )}>
