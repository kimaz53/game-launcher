...
function ExecutableIcon({ relPath, alt }: { relPath?: string; alt: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Outside Wails we can render the portable PNG directly.
      if (!hasWailsApp()) {
        setSrc(`/${relPath}`)
        return
      }
  ...
  return (
    ...
    >
      <DialogContent className="max-w-2xl border-theme-border bg-theme-sidebar p-6 text-theme-text">
        ...
        <div className="max-h-[360px] overflow-auto rounded-lg border border-theme-border bg-theme-app mt-3">
...
export default function QuickAccessPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      // Clean up invalid/stale IDs so the persisted order stays consistent.
      const same =
            ...
            <div className="flex flex-wrap items-start gap-3 justify-center">
                ...
                return (
                  <div
                    ...
                    className={[
                      ...
                      isHovered ? 'ring-2 ring-theme-primary shadow-[0_0_0_1px_var(--color-border)]' : '',
                    ].join(' ')}
                    onDragOver={(e) => {
                      e.preventDefault()
                    }}
                    onDragEnter={(e) => {
                      ...
                      // Trello-like: reorder immediately on hover (swap preview becomes real).
                      // Always evaluate hover targets so users can move right, then back left,
                      // and re-enter previously hovered items without getting "stuck".
                  ...
                  >
                    <Button
                      ...
                      variant="secondary"
                      size="icon"
                      className="absolute right-1 top-1 z-10 h-7 w-7 rounded-md bg-theme-secondary text-theme-text opacity-0 transition-opacity group-hover:opacity-100 hover:bg-theme-error"
                      onClick={async (e) => {
                        e.preventDefault()
                        e.stopPropagation()
              ...
              <Button
                ...
                className={[
                  ...
                  'transition-colors hover:bg-theme-sidebar hover:border-theme-secondary',
                  quickAccessGames.length === 0 ? 'w-[680px] rounded-xl h-12' : 'h-16 w-16',
                ].join(' ')}
